import { Helmet } from "react-helmet-async";
import CorporateHeader from "../../../Components/Corporate-header";
import Footer from "../../../Components/Footer";

const ExportLoan = () => {
  return (
    <>
      <Helmet>
        {/* Mirrored from mcb.mu/corporate/finance-lending/corporate-lending/export-loan by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:50:59 GMT */}
        <meta name="title" content="Export Loan | Corporate Banking | MCB	" />
        <meta
          name="description"
          content="Get immediate export financing and improve cash flow with tailored solutions. Contact your dedicated Relationship Manager today"
        />
        <title>Export Loan | Corporate Banking | MCB</title>
        <meta
          property="og:title"
          content="Export Loan | Corporate Banking | MCB"
        />
        <meta
          property="og:description"
          content="Get immediate export financing and improve cash flow with tailored solutions. Contact your dedicated Relationship Manager today"
        />
        <link
          rel="canonical"
          href="/corporate/finance-lending/corporate-lending/export-loan"
        />
      </Helmet>
      <div className="body detail-landing">
        {/* Google Tag Manager (noscript) */}
        <noscript>
          <iframe
            src="https://www.googletagmanager.com/ns.html?id=GTM-NMCB2SN"
            height="0"
            width="0"
            style={{ display: "none", visibility: "hidden" }}
          ></iframe>
        </noscript>
        {/* End Google Tag Manager (noscript) */}
        {/* Header */}
        <CorporateHeader />
        {/* Header */}
        <div role="main" className="main">
          <section
            className="banner-small custom-bg-color-light-2 position-relative border-0 pb-sm-0 pt-lg-4 m-0 mt-80"
            style={{
              backgroundImage:
                "url(/images/mcb/corporate/banners/corporate-export-loan5a8a.jpeg?sfvrsn=61b97e03_1)",
            }}
          >
            <div className="overlay" />
            <div className="container position-relative custom-negative-margin-1 z-index-3 pb-lg-5 pb-sm-0 mb-lg-3">
              <div className="row d-flex">
                <div
                  className="col-lg-6 mb-5 mb-lg-0 appear-animation order-2 order-lg-1"
                  data-appear-animation="fadeInRightShorter"
                  data-appear-animation-delay={450}
                >
                  <h1
                    className="font-weight-semi-bold text-10 line-height-1 mb-3 pe-5 me-5"
                    style={{ color: "#ffffff" }}
                  >
                    Export Loan
                  </h1>
                  <p
                    className="lead line-height-2 text-4 pe-5 mb-4 pb-2"
                    style={{ color: "#ffffff" }}
                  >
                    Improve your cash flow with export discounting of your
                    invoices.
                  </p>
                </div>
                <div className="col-lg-6 text-center text-lg-start order-1 order-lg-2"></div>
              </div>
            </div>
          </section>
          <div className="row" data-sf-element="Row">
            <div
              id="Main_C002_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div
                id="Main_C004_Col00"
                className="sf_colsIn container"
                data-sf-element="Container"
                data-placeholder-label="Container"
              >
                <div className="check-ul-styling">
                  <div>
                    <h2>A flexible facility to ease your cash flow</h2>
                    <p>
                      Access immediate export financing to manage your cash
                      fluctuations through your dedicated Relationship Manager
                      who will provide you individually tailored solutions to
                      meet your specific business and your trade needs.
                    </p>
                    <h3>
                      <br />
                      How we can help your business?
                    </h3>
                    <p>
                      Take advantage of our flexible export financing to boost
                      your cash flow or respond to your company’s working
                      capital requirements:
                    </p>
                    <ul>
                      <li>
                        A dedicated Relationship Manager who provides you with a
                        personalised service
                      </li>
                      <li>
                        Tailor-made advice to suit your business’ requirements
                      </li>
                      <li>
                        Flexible funding matching your customers payment date
                      </li>
                      <li>
                        Boost your company’s cash flow by securing funding to
                        grow your business
                      </li>
                      <li>
                        Possibility to secure the risk of non payment with our
                        bespoke Credit Protection solution
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row mt-50" data-sf-element="Row">
            <div
              id="Main_C003_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div className="container">
                <div className="row">
                  <div className="col">
                    <div className="card card-cta-red border-radius-0 bg-color-light box-shadow-1 box-shadow-1-hover">
                      <div className="card-body">
                        <h3 className="card-title mb-1  font-weight-normal text-7 text-transform-none">
                          Talk to us
                        </h3>
                        <p className="card-text mt-3 lead font-weight-medium text-4">
                          Our team of specialists can offer you the best
                          possible advice regarding your financing needs.
                        </p>
                        <a
                          href="/contact-us"
                          target="_blank"
                          className="mb-1 mt-1 me-1 btn btn-primary"
                        >
                          Contact us{" "}
                          <i className="fas fa-angle-right icons ms-1" />{" "}
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        {/* Footer */}
        <Footer />
        {/* Footer */}
      </div>{" "}
      {/* Vendor */} {/* Theme Base, Components and Settings */}{" "}
      {/* Current Page Vendor and Views */} {/* Demo */} {/* Theme Custom */}{" "}
      {/* Theme Initialization Files */}
      {/* Mirrored from mcb.mu/corporate/finance-lending/corporate-lending/export-loan by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:51:00 GMT */}
    </>
  );
};
export default ExportLoan;
