/* ------------------------------------------------------------------
  ...
  Author:          MooTools, My Object Oriented Javascript Tools. Copyright (c) 2006-2007 Valerio Proietti, <https://mad4milk.net>, MIT Style License.
  Description:     JavaScript library allowing powerful, flexible, and cross-browser code
  URL:						 See https://www.mootools.net/ for more info
...
}return A;},getItems:function(A,B,C){return(C)?A:$$.unique(A);}},resolver:function(A){return(A=="xhtml")?"https://www.w3.org/1999/xhtml":false;},getElementsByTagName:function(D,C){var E=[];
...
}}.periodical(50);}else{if(document.readyState&&window.ie){if(!$("ie_ready")){var C=(window.location.protocol=="support_insurances.html")?"://0":"javascript:void(0)";document.write('<script id="ie_ready" defer src="'+C+'"><\/script>');
...
this.wrapper=new Element("div").inject(this.toolTip);$$(B).each(this.build,this);if(this.options.initialize){this.options.initialize.call(this);}},build:function(B){B.$tmp.myTitle=(B.href&&B.getTag()=="a")?B.href.replace("https://",""):(B.rel||false);
