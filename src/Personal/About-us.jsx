import { Helmet } from "react-helmet-async";
import PersonalHeader from "../Components/Personal-header";
import Footer from "../Components/Footer";

const AboutUs = () => {
  return (
    <>
      <Helmet>
        {/* Mirrored from mcb.mu/personal/about-us by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:53:22 GMT */}
        <meta name="title" content="About Us | MCB	" />
        <meta
          name="description"
          content="MCB offers banking solutions tailored for each customer, assisting in the advancement of individuals and corporations. Find out how we can help you!"
        />
        <title>About Us | MCB</title>
        <meta property="og:title" content="About Us | MCB" />
        <meta
          property="og:description"
          content="MCB offers banking solutions tailored for each customer, assisting in the advancement of individuals and corporations. Find out how we can help you!"
        />
        <meta property="og:url" content="https://mcb.mu/personal/about-us" />
        <link rel="canonical" href="/personal/about-us" />
      </Helmet>
      <div className="body detail-landing">
        {/* Google Tag Manager (noscript) */}
        <noscript>
          <iframe
            src="https://www.googletagmanager.com/ns.html?id=GTM-NMCB2SN"
            height="0"
            width="0"
            style={{ display: "none", visibility: "hidden" }}
          ></iframe>
        </noscript>
        {/* End Google Tag Manager (noscript) */}
        {/* Header */}
        <PersonalHeader />
        {/* Header */}
        <div role="main" className="main">
          <section
            className="banner-small custom-bg-color-light-2 position-relative border-0 pb-sm-0 pt-lg-4 m-0 mt-80"
            style={{
              backgroundImage:
                "url(/images/mcb/about-us/mcb-about-us30fc.jpeg?sfvrsn=c6a632b2_1)",
            }}
          >
            <div className="overlay" />
            <div className="container position-relative custom-negative-margin-1 z-index-3 pb-lg-5 pb-sm-0 mb-lg-3">
              <div className="row d-flex">
                <div
                  className="col-lg-6 mb-5 mb-lg-0 appear-animation order-2 order-lg-1"
                  data-appear-animation="fadeInRightShorter"
                  data-appear-animation-delay={450}
                >
                  <h1
                    className="font-weight-semi-bold text-10 line-height-1 mb-3 pe-5 me-5"
                    style={{ color: "#ffffff" }}
                  >
                    About us
                  </h1>
                </div>
                <div className="col-lg-6 text-center text-lg-start order-1 order-lg-2"></div>
              </div>
            </div>
          </section>
          <div className="container py-5">
            <div className="row py-3 justify-content-start">
              <div className="col-lg-12 "></div>
              <div className="feature_three row">
                <div
                  className="col-sm-12 col-md-6 mb-4 appear-animation animated fadeIn appear-animation-visible"
                  data-appear-animation="fadeIn"
                  style={{ animationDelay: "100ms" }}
                >
                  <article className="faq">
                    <div className="border boxshadow px-3 py-2">
                      <a
                        href="/personal/about-us/at-a-glance"
                        className="text-3 text-uppercase font-weight-medium d-block text-dark text-decoration-none d-flex align-items-center justify-content-between"
                      >
                        <h4 className="mb-0">
                          <span>At a glance</span>
                        </h4>
                        <i className="fas fa-chevron-right" />
                      </a>
                    </div>
                  </article>
                </div>
                <div
                  className="col-sm-12 col-md-6 mb-4 appear-animation animated fadeIn appear-animation-visible"
                  data-appear-animation="fadeIn"
                  style={{ animationDelay: "100ms" }}
                >
                  <article className="faq">
                    <div className="border boxshadow px-3 py-2">
                      <a
                        href="/personal/about-us/philosophy-vision"
                        className="text-3 text-uppercase font-weight-medium d-block text-dark text-decoration-none d-flex align-items-center justify-content-between"
                      >
                        <h4 className="mb-0">
                          <span>Philosophy &amp; Vision</span>
                        </h4>
                        <i className="fas fa-chevron-right" />
                      </a>
                    </div>
                  </article>
                </div>
                <div
                  className="col-sm-12 col-md-6 mb-4 appear-animation animated fadeIn appear-animation-visible"
                  data-appear-animation="fadeIn"
                  style={{ animationDelay: "100ms" }}
                >
                  <article className="faq">
                    <div className="border boxshadow px-3 py-2">
                      <a
                        href="/personal/about-us/governance"
                        className="text-3 text-uppercase font-weight-medium d-block text-dark text-decoration-none d-flex align-items-center justify-content-between"
                      >
                        <h4 className="mb-0">
                          <span>Governance</span>
                        </h4>
                        <i className="fas fa-chevron-right" />
                      </a>
                    </div>
                  </article>
                </div>
                <div
                  className="col-sm-12 col-md-6 mb-4 appear-animation animated fadeIn appear-animation-visible"
                  data-appear-animation="fadeIn"
                  style={{ animationDelay: "100ms" }}
                >
                  <article className="faq">
                    <div className="border boxshadow px-3 py-2">
                      <a
                        href="/personal/about-us/leadership-team"
                        className="text-3 text-uppercase font-weight-medium d-block text-dark text-decoration-none d-flex align-items-center justify-content-between"
                      >
                        <h4 className="mb-0">
                          <span>Leadership Team</span>
                        </h4>
                        <i className="fas fa-chevron-right" />
                      </a>
                    </div>
                  </article>
                </div>
                <div
                  className="col-sm-12 col-md-6 mb-4 appear-animation animated fadeIn appear-animation-visible"
                  data-appear-animation="fadeIn"
                  style={{ animationDelay: "100ms" }}
                >
                  <article className="faq">
                    <div className="border boxshadow px-3 py-2">
                      <a
                        href="/personal/about-us/financials-annual-reports"
                        className="text-3 text-uppercase font-weight-medium d-block text-dark text-decoration-none d-flex align-items-center justify-content-between"
                      >
                        <h4 className="mb-0">
                          <span>Financial &amp; Annual Reports</span>
                        </h4>
                        <i className="fas fa-chevron-right" />
                      </a>
                    </div>
                  </article>
                </div>
                <div
                  className="col-sm-12 col-md-6 mb-4 appear-animation animated fadeIn appear-animation-visible"
                  data-appear-animation="fadeIn"
                  style={{ animationDelay: "100ms" }}
                >
                  <article className="faq">
                    <div className="border boxshadow px-3 py-2">
                      <a
                        href="/personal/about-us/lcr-disclosures"
                        className="text-3 text-uppercase font-weight-medium d-block text-dark text-decoration-none d-flex align-items-center justify-content-between"
                      >
                        <h4 className="mb-0">
                          <span>LCR Disclosures</span>
                        </h4>
                        <i className="fas fa-chevron-right" />
                      </a>
                    </div>
                  </article>
                </div>
              </div>
            </div>
          </div>
        </div>
        {/* Footer */}
        <Footer />
        {/* Footer */}
      </div>{" "}
      {/* Vendor */} {/* Theme Base, Components and Settings */}{" "}
      {/* Current Page Vendor and Views */} {/* Demo */} {/* Theme Custom */}{" "}
      {/* Theme Initialization Files */}
      {/* Mirrored from mcb.mu/personal/about-us by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:53:25 GMT */}
    </>
  );
};
export default AboutUs;
