import { Helmet } from "react-helmet-async";
import CorporateHeader from "../../../Components/Corporate-header";
import Footer from "../../../Components/Footer";

const CustodyServices = () => {
  return (
    <>
      <Helmet>
        {/* Mirrored from mcb.mu/corporate/what-we-do/global-business/custody-services by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:47:25 GMT */}
        <meta
          name="title"
          content="Custody Services | Corporate Banking | MCB"
        />
        <meta
          name="description"
          content="Explore MCB's 25+ years of premier custody services for global investors and financial institutions, offering access to securities markets worldwide."
        />
        <title>Custody Services | Corporate Banking | MCB</title>
        <meta
          property="og:title"
          content="Custody Services | Corporate Banking | MCB"
        />
        <meta
          property="og:description"
          content="Explore MCB's 25+ years of premier custody services for global investors and financial institutions, offering access to securities markets worldwide."
        />
        <meta
          property="og:url"
          content="https://mcb.mu/corporate/what-we-do/global-business/custody-services"
        />
        <link
          rel="canonical"
          href="/corporate/what-we-do/global-business/custody-services"
        />
      </Helmet>
      <div className="body detail-landing">
        {/* Google Tag Manager (noscript) */}
        <noscript>
          <iframe
            src="https://www.googletagmanager.com/ns.html?id=GTM-NMCB2SN"
            height="0"
            width="0"
            style={{ display: "none", visibility: "hidden" }}
          ></iframe>
        </noscript>
        {/* End Google Tag Manager (noscript) */}
        {/* Header */}
        <CorporateHeader />
        {/* Header */}
        <div role="main" className="main">
          <section
            className="banner-small custom-bg-color-light-2 position-relative border-0 pb-sm-0 pt-lg-4 m-0 mt-80"
            style={{
              backgroundImage:
                "url(/images/mcb/corporate/banners/corporate-global-business-custody-services8982f8859590420183614a43e7b907161230.jpeg?sfvrsn=5612ba92_0)",
            }}
          >
            <div className="overlay" />
            <div className="container position-relative custom-negative-margin-1 z-index-3 pb-lg-5 pb-sm-0 mb-lg-3">
              <div className="row d-flex">
                <div
                  className="col-lg-6 mb-5 mb-lg-0 appear-animation order-2 order-lg-1"
                  data-appear-animation="fadeInRightShorter"
                  data-appear-animation-delay={450}
                >
                  <h1
                    className="font-weight-semi-bold text-10 line-height-1 mb-3 pe-5 me-5"
                    style={{ color: "#ffffff" }}
                  >
                    Custody services
                  </h1>
                  <p
                    className="lead line-height-2 text-4 pe-5 mb-4 pb-2"
                    style={{ color: "#ffffff" }}
                  >
                    The leading financial services provider with a dedicated
                    relationship management team providing clients with a
                    gateway to local, regional and international securities
                    markets.
                  </p>
                </div>
                <div className="col-lg-6 text-center text-lg-start order-1 order-lg-2"></div>
              </div>
            </div>
          </section>
          <div className="row" data-sf-element="Row">
            <div
              id="Main_C003_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div
                id="Main_C004_Col00"
                className="sf_colsIn container"
                data-sf-element="Container"
                data-placeholder-label="Container"
              >
                <div className="check-ul-styling">
                  <div>
                    <h2>A local pioneer with international reach</h2>
                    <p>
                      With over 25 years of industry expertise and a global
                      footprint in over 75 countries, MCB is the largest
                      provider of securities services in Mauritius, offering
                      international custody to both domestic and foreign
                      investors, through an selective network of third-party
                      providers.
                    </p>
                    <h3>How we can help your business</h3>
                    <p>
                      Building on our rich experience, proven expertise and
                      coupled with our state-of-the-art technology, we offer
                      value-added services at some of the most competitive rates
                      on the market.
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row" data-sf-element="Row">
            <div
              id="Main_C002_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div className="container">
                <div className="row">
                  <div className="col">
                    <div className="card card-cta-red border-radius-0 bg-color-light box-shadow-1 box-shadow-1-hover">
                      <div className="card-body">
                        <h3 className="card-title mb-1  font-weight-normal text-7 text-transform-none">
                          Talk to us
                        </h3>
                        <p className="card-text mt-3 lead font-weight-medium text-4">
                          Our team of specialists can offer you the best
                          possible advice regarding your securities services
                          requirement.
                        </p>
                        <a
                          href="/contact-us"
                          target="_blank"
                          className="mb-1 mt-1 me-1 btn btn-primary"
                        >
                          Contact us{" "}
                          <i className="fas fa-angle-right icons ms-1" />{" "}
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        {/* Footer */}
        <Footer />
        {/* Footer */}
      </div>{" "}
      {/* Vendor */} {/* Theme Base, Components and Settings */}{" "}
      {/* Current Page Vendor and Views */} {/* Demo */} {/* Theme Custom */}{" "}
      {/* Theme Initialization Files */}
      {/* Mirrored from mcb.mu/corporate/what-we-do/global-business/custody-services by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:47:30 GMT */}
    </>
  );
};
export default CustodyServices;
