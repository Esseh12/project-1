import { Helmet } from "react-helmet-async";
import CorporateHeader from "../../Components/Corporate-header";
import Footer from "../../Components/Footer";

const CorporateFactsFigures = () => {
  return (
    <>
      <Helmet>
        {/* Mirrored from mcb.mu/corporate/mcb-at-a-glance/facts-figures by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 09:25:35 GMT */}
        <meta name="title" content="Facts &amp; Figures" />
        <title>Facts &amp; Figures</title>
        <meta property="og:title" content="Facts & Figures" />
        <meta
          property="og:url"
          content="https://mcb.mu/corporate/mcb-at-a-glance/facts-figures"
        />
        <meta property="og:type" content="website" />
        <meta property="og:site_name" content="MCB Mauritius" />
        <link rel="canonical" href="/facts-figures" />
      </Helmet>
      <div className="body detail-landing">
        {/* Google Tag Manager (noscript) */}
        <noscript>
          <iframe
            src="https://www.googletagmanager.com/ns.html?id=GTM-NMCB2SN"
            height="0"
            width="0"
            style={{ display: "none", visibility: "hidden" }}
          ></iframe>
        </noscript>
        {/* End Google Tag Manager (noscript) */}
        {/* Header */}
        <CorporateHeader />
        {/* Header */}
        <div role="main" className="main">
          <section
            className="banner-small custom-bg-color-light-2 position-relative border-0 pb-sm-0 pt-lg-4 m-0 mt-80"
            style={{
              backgroundImage:
                "url(/images/mcb/corporate/banners/corporate-mcb-at-a-glancec2ef.jpeg?sfvrsn=5edca2d6_1)",
            }}
          >
            <div className="overlay" />
            <div className="container position-relative custom-negative-margin-1 z-index-3 pb-lg-5 pb-sm-0 mb-lg-3">
              <div className="row d-flex">
                <div
                  className="col-lg-6 mb-5 mb-lg-0 appear-animation order-2 order-lg-1"
                  data-appear-animation="fadeInRightShorter"
                  data-appear-animation-delay={450}
                >
                  <h1
                    className="font-weight-semi-bold text-10 line-height-1 mb-3 pe-5 me-5"
                    style={{ color: "#ffffff" }}
                  >
                    MCB at a glance
                  </h1>
                  <div className="col mt-4">
                    <a
                      href="../our-geographical-presence.html"
                      className=" btn btn-primary me-4 mt-3 w-100-mobile btn-floating is-visible"
                      target="_blank"
                      style={{ color: "#ffffff" }}
                    >
                      View our geographical presence
                      <i className="fas fa-angle-right icons ms-1" />{" "}
                    </a>
                  </div>
                </div>
                <div className="col-lg-6 text-center text-lg-start order-1 order-lg-2"></div>
              </div>
            </div>
          </section>
          <div className="row" data-sf-element="Row">
            <div
              id="Main_C010_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div
                id="Main_C011_Col00"
                className="sf_colsIn container"
                data-sf-element="Container"
                data-placeholder-label="Container"
              >
                <div className="check-ul-styling">
                  <div>
                    <h2>Facts &amp; Figures</h2>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row" data-sf-element="Row">
            <div
              id="Main_C005_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div
                id="Main_C006_Col00"
                className="sf_colsIn container"
                data-sf-element="Container"
                data-placeholder-label="Container"
              >
                <div className="row" data-sf-element="Row">
                  <div
                    id="Main_C007_Col00"
                    className="sf_colsIn col-lg-6"
                    data-sf-element="Column 1"
                    data-placeholder-label="Column 1"
                  >
                    <div className="container">
                      <div className="row">
                        <div className="col">
                          <div className="card card-cta-red border-radius-0 bg-color-light box-shadow-1 box-shadow-1-hover">
                            <div className="card-body">
                              <h3 className="card-title mb-1  font-weight-normal text-7 text-transform-none">
                                MCB Group
                              </h3>
                              <p className="card-text mt-3 lead font-weight-medium text-4">
                                Annual Report &amp; Financial Statements
                              </p>
                              <a
                                href="https://mcbgroup.com/investors-centre/financials-annual-reports"
                                target="_blank"
                                className="mb-1 mt-1 me-1 btn btn-primary"
                              >
                                View{" "}
                                <i className="fas fa-angle-right icons ms-1" />{" "}
                              </a>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div
                    id="Main_C007_Col01"
                    className="sf_colsIn col-lg-6"
                    data-sf-element="Column 2"
                    data-placeholder-label="Column 2"
                  >
                    <div className="container">
                      <div className="row">
                        <div className="col">
                          <div className="card card-cta-red border-radius-0 bg-color-light box-shadow-1 box-shadow-1-hover">
                            <div className="card-body">
                              <h3 className="card-title mb-1  font-weight-normal text-7 text-transform-none">
                                MCB Ltd
                              </h3>
                              <p className="card-text mt-3 lead font-weight-medium text-4">
                                Annual Report &amp; Financial Statements
                              </p>
                              <a
                                href="/personal/about-us/financial-reports.html"
                                target="_blank"
                                className="mb-1 mt-1 me-1 btn btn-primary"
                              >
                                View{" "}
                                <i className="fas fa-angle-right icons ms-1" />{" "}
                              </a>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row mt-50" data-sf-element="Row">
            <div
              id="Main_C002_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div
                id="Main_C003_Col00"
                className="sf_colsIn container"
                data-sf-element="Container"
                data-placeholder-label="Container"
              >
                <div className="container my-5">
                  <div className="row">
                    <div className="col">
                      <div className="tabs tabs-bottom tabs-left tabs-simple">
                        <ul
                          className="nav nav-tabs justify-content-start"
                          role="tablist"
                        >
                          <li
                            className="nav-item active w-100-mobile"
                            role="presentation"
                            id="id"
                          >
                            <a
                              className="nav-link active"
                              href="#tabs-financial-year-2019"
                              data-bs-toggle="tab"
                              aria-selected="true"
                              role="tab"
                            >
                              Financial Year 2019
                            </a>
                          </li>
                          <li
                            className="nav-item  w-100-mobile"
                            role="presentation"
                            id="id"
                          >
                            <a
                              className="nav-link "
                              href="#tabs-financial-year-2018"
                              data-bs-toggle="tab"
                              aria-selected="true"
                              role="tab"
                            >
                              Financial Year 2018
                            </a>
                          </li>
                          <li
                            className="nav-item  w-100-mobile"
                            role="presentation"
                            id="id"
                          >
                            <a
                              className="nav-link "
                              href="#tabs-financial-year-2017"
                              data-bs-toggle="tab"
                              aria-selected="true"
                              role="tab"
                            >
                              Financial year 2017
                            </a>
                          </li>
                          <li
                            className="nav-item  w-100-mobile"
                            role="presentation"
                            id="id"
                          >
                            <a
                              className="nav-link "
                              href="#tabs-financial-year-2016"
                              data-bs-toggle="tab"
                              aria-selected="true"
                              role="tab"
                            >
                              Financial year 2016
                            </a>
                          </li>
                          <li
                            className="nav-item  w-100-mobile"
                            role="presentation"
                            id="id"
                          >
                            <a
                              className="nav-link "
                              href="#tabs-financial-year-2015"
                              data-bs-toggle="tab"
                              aria-selected="true"
                              role="tab"
                            >
                              Financial year 2015
                            </a>
                          </li>
                        </ul>
                        <div className="tab-content">
                          <div
                            className="tab-pane active show"
                            id="tabs-financial-year-2019"
                            role="tabpanel"
                          >
                            <div className="text-left">
                              <h2 className="text-7 font-weight-normal" />
                              <div>
                                <table
                                  className="factstable k-table"
                                  width="98%"
                                  cellSpacing={0}
                                  cellPadding={8}
                                >
                                  <tbody>
                                    <tr className="redbg">
                                      <td>
                                        <strong>
                                          Few Facts &amp; Figures - 2019
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>MCB Group</strong>
                                      </td>
                                      <td>
                                        <strong>MCB Ltd</strong>
                                      </td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>Our Operations</strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Total assets &nbsp;</td>
                                      <td>USD 13.4 Bn (MUR 471 Bn)</td>
                                      <td>USD 12.2 Bn (MUR 428 Bn)</td>
                                    </tr>
                                    <tr>
                                      <td>Customers</td>
                                      <td>~ 1,062,100</td>
                                      <td>~ 1,005,300</td>
                                    </tr>
                                    <tr>
                                      <td>Branches</td>
                                      <td>60</td>
                                      <td>40</td>
                                    </tr>
                                    <tr>
                                      <td>Employees</td>
                                      <td>3,617</td>
                                      <td>2,861</td>
                                    </tr>
                                    <tr>
                                      <td>ATMs</td>
                                      <td>206</td>
                                      <td>177</td>
                                    </tr>
                                    <tr>
                                      <td>Correspondent Banks</td>
                                      <td>1,150</td>
                                      <td>1,150</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Stock Profile (MCB Group)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Market Capitalisation</td>
                                      <td colSpan={2}>
                                        USD 1.96 Bn &nbsp; (MUR 69 Bn)
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Shareholder base</td>
                                      <td colSpan={2}>&gt; 20,000</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Investment Grade Ratings (MCB Ltd)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Moody’s</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Long Term</td>
                                      <td colSpan={2}>Baa2</td>
                                    </tr>
                                    <tr>
                                      <td>Short Term</td>
                                      <td colSpan={2}>P-2</td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Fitch Ratings</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Long Term</td>
                                      <td colSpan={2}>BBB-</td>
                                    </tr>
                                    <tr>
                                      <td>Short Term</td>
                                      <td colSpan={2}>F3</td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>
                                            Care Ratings (Africa) Private
                                            Limited
                                          </em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Rating</td>
                                      <td colSpan={2}>AAA*</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </div>
                            </div>
                          </div>
                          <div
                            className="tab-pane "
                            id="tabs-financial-year-2018"
                            role="tabpanel"
                          >
                            <div className="text-left">
                              <h2 className="text-7 font-weight-normal" />
                              <div>
                                <table
                                  className="factstable"
                                  width="98%"
                                  cellSpacing={0}
                                  cellPadding={8}
                                >
                                  <tbody>
                                    <tr className="redbg">
                                      <td>
                                        <strong>
                                          Few Facts &amp; Figures - 2018
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>MCB Group</strong>
                                      </td>
                                      <td>
                                        <strong>MCB Ltd</strong>
                                      </td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>Our Operations</strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Total assets &nbsp;</td>
                                      <td>USD 11.2 Bn (MUR 386 Bn)</td>
                                      <td>USD 10.1 Bn (MUR 350 Bn)</td>
                                    </tr>
                                    <tr>
                                      <td>Customers</td>
                                      <td>~ 1,028,300</td>
                                      <td>~ 976,700</td>
                                    </tr>
                                    <tr>
                                      <td>Branches</td>
                                      <td>55</td>
                                      <td>39</td>
                                    </tr>
                                    <tr>
                                      <td>Employees</td>
                                      <td>~ 3,409</td>
                                      <td>~ 2,696</td>
                                    </tr>
                                    <tr>
                                      <td>ATMs</td>
                                      <td>201</td>
                                      <td>177</td>
                                    </tr>
                                    <tr>
                                      <td>Correspondent Banks</td>
                                      <td>1,200</td>
                                      <td>1,200</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Stock Profile (MCB Group)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Market Capitalisation</td>
                                      <td colSpan={2}>
                                        USD 1.9 Bn &nbsp; (MUR 65.9 Bn)
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Shareholder base</td>
                                      <td colSpan={2}>&gt; 19,000</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Investment Grade Ratings (MCB Ltd)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Moody’s</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Positive</td>
                                    </tr>
                                    <tr>
                                      <td>Long Term</td>
                                      <td colSpan={2}>Baa3</td>
                                    </tr>
                                    <tr>
                                      <td>Short Term</td>
                                      <td colSpan={2}>P-3</td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Fitch Ratings</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Long Term</td>
                                      <td colSpan={2}>BBB-</td>
                                    </tr>
                                    <tr>
                                      <td>Short Term</td>
                                      <td colSpan={2}>F3</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </div>
                            </div>
                          </div>
                          <div
                            className="tab-pane "
                            id="tabs-financial-year-2017"
                            role="tabpanel"
                          >
                            <div className="text-left">
                              <h2 className="text-7 font-weight-normal" />
                              <div>
                                <table
                                  className="factstable"
                                  width="98%"
                                  cellSpacing={0}
                                  cellPadding={8}
                                >
                                  <tbody>
                                    <tr className="redbg">
                                      <td>
                                        <strong>
                                          Few Facts &amp; Figures - 2017
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>MCB Group</strong>
                                      </td>
                                      <td>
                                        <strong>MCB Ltd</strong>
                                      </td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>Our Operations</strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Total assets &nbsp;</td>
                                      <td>
                                        USD 9.8 Bn &nbsp; &nbsp;(MUR 345.7 Bn)
                                      </td>
                                      <td>USD 8.9 Bn (MUR 314 Bn)</td>
                                    </tr>
                                    <tr>
                                      <td>Customers</td>
                                      <td>&nbsp;~ 1,010,700</td>
                                      <td>~ 960,000</td>
                                    </tr>
                                    <tr>
                                      <td>Branches</td>
                                      <td>54</td>
                                      <td>39</td>
                                    </tr>
                                    <tr>
                                      <td>Employees</td>
                                      <td>~ 3,400</td>
                                      <td>~ &nbsp;2,682</td>
                                    </tr>
                                    <tr>
                                      <td>ATMs</td>
                                      <td>198</td>
                                      <td>176</td>
                                    </tr>
                                    <tr>
                                      <td>Correspondent Banks</td>
                                      <td>~&nbsp; 1,470</td>
                                      <td>~&nbsp; 1,470</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Stock Profile (MCB Group)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Market Capitalisation</td>
                                      <td colSpan={2}>
                                        USD 1.9 Bn &nbsp; (MUR 68.4 Bn)
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Shareholder base</td>
                                      <td colSpan={2}>&gt; 18,500</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Investment Grade Ratings (MCB Ltd)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Moody’s</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Bank Deposits</td>
                                      <td colSpan={2}>Baa3/P-3</td>
                                    </tr>
                                    <tr>
                                      <td>Baseline Credit Assessment</td>
                                      <td colSpan={2}>ba1</td>
                                    </tr>
                                    <tr>
                                      <td>
                                        Adjusted Baseline Credit Assessment
                                      </td>
                                      <td colSpan={2}>ba1</td>
                                    </tr>
                                    <tr>
                                      <td>Issuer Rating</td>
                                      <td colSpan={2}>Baa3</td>
                                    </tr>
                                    <tr>
                                      <td>Counterparty Risk Assessment</td>
                                      <td colSpan={2}>Baa2/P-2</td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Fitch Ratings</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Long-Term Issuer Default Rating</td>
                                      <td colSpan={2}>BBB-</td>
                                    </tr>
                                    <tr>
                                      <td>Short-Term Issuer Default Rating</td>
                                      <td colSpan={2}>F3</td>
                                    </tr>
                                    <tr>
                                      <td>Viability Rating</td>
                                      <td colSpan={2}>bbb-</td>
                                    </tr>
                                    <tr>
                                      <td>Support Rating</td>
                                      <td colSpan={2}>3</td>
                                    </tr>
                                    <tr>
                                      <td>Support Rating Floor</td>
                                      <td colSpan={2}>BB+</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </div>
                            </div>
                          </div>
                          <div
                            className="tab-pane "
                            id="tabs-financial-year-2016"
                            role="tabpanel"
                          >
                            <div className="text-left">
                              <h2 className="text-7 font-weight-normal" />
                              <div>
                                <table
                                  className="factstable k-table"
                                  width="98%"
                                  cellSpacing={0}
                                  cellPadding={8}
                                >
                                  <tbody>
                                    <tr className="redbg">
                                      <td>
                                        <strong>
                                          Few Facts &amp; Figures - 2016
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>MCB Group</strong>
                                      </td>
                                      <td>
                                        <strong>MCB Ltd</strong>
                                      </td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>Our Operations</strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Total assets</td>
                                      <td>USD 8.7 Bn (MUR 318 Bn)</td>
                                      <td>USD 7.8 Bn (MUR 286 Bn) &nbsp;</td>
                                    </tr>
                                    <tr>
                                      <td>Customers</td>
                                      <td>&nbsp;~988,000</td>
                                      <td>~ 940,000</td>
                                    </tr>
                                    <tr>
                                      <td>Branches</td>
                                      <td>40</td>
                                      <td>40</td>
                                    </tr>
                                    <tr>
                                      <td>Employees</td>
                                      <td>~ 3,220</td>
                                      <td>~ &nbsp;2,600</td>
                                    </tr>
                                    <tr>
                                      <td>ATMs</td>
                                      <td>194</td>
                                      <td>173</td>
                                    </tr>
                                    <tr>
                                      <td>Correspondent Banks</td>
                                      <td>~&nbsp; 1,600</td>
                                      <td>~&nbsp; 1,600</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Stock Profile (MCB Group)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Market Capitalisation</td>
                                      <td colSpan={2}>
                                        USD 1.4 Bn &nbsp; (MUR 50.3 Bn)
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Shareholder base</td>
                                      <td colSpan={2}>&gt; 18,000</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Investment Grade Ratings (MCB Ltd)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Moody’s</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Bank Deposits</td>
                                      <td colSpan={2}>Baa3/P-3</td>
                                    </tr>
                                    <tr>
                                      <td>Baseline Credit Assessment</td>
                                      <td colSpan={2}>ba1</td>
                                    </tr>
                                    <tr>
                                      <td>
                                        Adjusted Baseline Credit Assessment
                                      </td>
                                      <td colSpan={2}>ba1</td>
                                    </tr>
                                    <tr>
                                      <td>Issuer Rating</td>
                                      <td colSpan={2}>Baa3</td>
                                    </tr>
                                    <tr>
                                      <td>Counterparty Risk Assessment</td>
                                      <td colSpan={2}>Baa2/P-2</td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Fitch Ratings</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Long-Term Issuer Default Rating</td>
                                      <td colSpan={2}>BBB-</td>
                                    </tr>
                                    <tr>
                                      <td>Short-Term Issuer Default Rating</td>
                                      <td colSpan={2}>F3</td>
                                    </tr>
                                    <tr>
                                      <td>Viability Rating</td>
                                      <td colSpan={2}>bbb-</td>
                                    </tr>
                                    <tr>
                                      <td>Support Rating</td>
                                      <td colSpan={2}>3</td>
                                    </tr>
                                    <tr>
                                      <td>Support Rating Floor</td>
                                      <td colSpan={2}>BB+</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </div>
                            </div>
                          </div>
                          <div
                            className="tab-pane "
                            id="tabs-financial-year-2015"
                            role="tabpanel"
                          >
                            <div className="text-left">
                              <h2 className="text-7 font-weight-normal" />
                              <div>
                                <table
                                  className="factstable k-table"
                                  width="98%"
                                  cellSpacing={0}
                                  cellPadding={8}
                                >
                                  <tbody>
                                    <tr className="redbg">
                                      <td>
                                        <strong>
                                          Few Facts &amp; Figures - 2015
                                        </strong>
                                      </td>
                                      <td>
                                        <strong>MCB Group</strong>
                                      </td>
                                      <td>
                                        <strong>MCB Ltd</strong>
                                      </td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>Our Operations</strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Total assets &nbsp;</td>
                                      <td>USD 7.8 Bn (MUR 280 Bn)</td>
                                      <td>USD 7.0 Bn (MUR 251 Bn)</td>
                                    </tr>
                                    <tr>
                                      <td>Customers</td>
                                      <td>&nbsp;~ 930,000</td>
                                      <td>~ 930,000</td>
                                    </tr>
                                    <tr>
                                      <td>Branches</td>
                                      <td>40</td>
                                      <td>40</td>
                                    </tr>
                                    <tr>
                                      <td>Employees</td>
                                      <td>~ 2,500</td>
                                      <td>~&nbsp; 2,500</td>
                                    </tr>
                                    <tr>
                                      <td>ATMs</td>
                                      <td>166</td>
                                      <td>166</td>
                                    </tr>
                                    <tr>
                                      <td>Correspondent Banks</td>
                                      <td>~&nbsp; 1,600</td>
                                      <td>~&nbsp; 1600</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Stock Profile (MCB Group)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Market Capitalisation</td>
                                      <td colSpan={2}>
                                        USD 1.4 Bn &nbsp; (MUR 51.7 Bn)
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Shareholder base</td>
                                      <td colSpan={2}>&gt; 18,000</td>
                                    </tr>
                                    <tr className="greybg">
                                      <td colSpan={3}>
                                        <strong>
                                          Our Investment Grade Ratings (MCB Ltd)
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Moody’s</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Bank Deposits</td>
                                      <td colSpan={2}>Baa3/P-3</td>
                                    </tr>
                                    <tr>
                                      <td>Baseline Credit Assessment</td>
                                      <td colSpan={2}>ba1</td>
                                    </tr>
                                    <tr>
                                      <td>
                                        Adjusted Baseline Credit Assessment
                                      </td>
                                      <td colSpan={2}>ba1</td>
                                    </tr>
                                    <tr>
                                      <td>Issuer Rating</td>
                                      <td colSpan={2}>Baa3</td>
                                    </tr>
                                    <tr>
                                      <td>Counterparty Risk Assessment</td>
                                      <td colSpan={2}>Baa2/P-2</td>
                                    </tr>
                                    <tr className="lgreybg">
                                      <td colSpan={3}>
                                        <strong>
                                          <em>Fitch Ratings</em>
                                        </strong>
                                      </td>
                                    </tr>
                                    <tr>
                                      <td>Outlook</td>
                                      <td colSpan={2}>Stable</td>
                                    </tr>
                                    <tr>
                                      <td>Long-Term Issuer Default Rating</td>
                                      <td colSpan={2}>BBB-</td>
                                    </tr>
                                    <tr>
                                      <td>Short-Term Issuer Default Rating</td>
                                      <td colSpan={2}>F3</td>
                                    </tr>
                                    <tr>
                                      <td>Viability Rating</td>
                                      <td colSpan={2}>bbb-</td>
                                    </tr>
                                    <tr>
                                      <td>Support Rating</td>
                                      <td colSpan={2}>3</td>
                                    </tr>
                                    <tr>
                                      <td>Support Rating Floor</td>
                                      <td colSpan={2}>BB+</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        {/* Footer */}
        <Footer />
        {/* Footer */}
      </div>{" "}
      {/* Vendor */} {/* Theme Base, Components and Settings */}{" "}
      {/* Current Page Vendor and Views */} {/* Demo */} {/* Theme Custom */}{" "}
      {/* Theme Initialization Files */}
      {/* Mirrored from mcb.mu/corporate/mcb-at-a-glance/facts-figures by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 09:25:36 GMT */}
    </>
  );
};
export default CorporateFactsFigures;
