import { Helmet } from "react-helmet-async";
import SmeHeader from "../Components/Sme-header";
import Footer from "../Components/Footer";

const SmeConnect = () => {
  return (
    <>
      <Helmet>
        {/* Mirrored from mcb.mu/sme/sme-connect by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:43:52 GMT */}
        <meta name="title" content="SME Connect | MCB	" />
        <meta
          name="description"
          content="Join MCB's Punch platform for SMEs to connect, find solutions, and access resources for business growth."
        />
        <title>SME Connect | MCB</title>
        <meta property="og:title" content="SME Connect | MCB" />
        <meta
          property="og:description"
          content="Join MCB's Punch platform for SMEs to connect, find solutions, and access resources for business growth."
        />
        <meta property="og:url" content="https://mcb.mu/sme/sme-connect" />
        <link rel="canonical" href="/sme-connect" />
      </Helmet>
      <div className="body detail-landing">
        {/* Google Tag Manager (noscript) */}
        <noscript
          dangerouslySetInnerHTML={{
            __html: `<iframe src="https://www.googletagmanager.com/ns.html?id=GTM-NMCB2SN" height="0" width="0" style="display:none;visibility:hidden"></iframe>`,
          }}
        />
        {/* End Google Tag Manager (noscript) */}
        {/* Header */}
        <SmeHeader />
        {/* Header */}
        <div role="main" className="main">
          <section
            className="banner-small custom-bg-color-light-2 position-relative border-0 pb-sm-0 pt-lg-4 m-0 mt-80"
            style={{
              backgroundImage:
                "url(/images/mcb/sme/banners/sme-connect2e21.jpeg?sfvrsn=d9b5bff3_0)",
            }}
          >
            <div className="overlay" />
            <div className="container position-relative custom-negative-margin-1 z-index-3 pb-lg-5 pb-sm-0 mb-lg-3">
              <div className="row d-flex">
                <div
                  className="col-lg-6 mb-5 mb-lg-0 appear-animation order-2 order-lg-1"
                  data-appear-animation="fadeInRightShorter"
                  data-appear-animation-delay={450}
                >
                  <h1
                    className="font-weight-semi-bold text-10 line-height-1 mb-3 pe-5 me-5"
                    style={{ color: "#ffffff" }}
                  >
                    SME Connect
                  </h1>
                  <p
                    className="lead line-height-2 text-4 pe-5 mb-4 pb-2"
                    style={{ color: "#ffffff" }}
                  >
                    Growing a successful business isn&#39;t and shouldn&#39;t be
                    a lonesome journey. At MCB, we believe that sustainable
                    growth rhymes with collaboration. That&#39;s why we do
                    everything in our power to connect you to the right people.
                  </p>
                </div>
                <div className="col-lg-6 text-center text-lg-start order-1 order-lg-2"></div>
              </div>
            </div>
          </section>
          <div className="row mt-50" data-sf-element="Row">
            <div
              id="Main_C002_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div
                id="Main_C003_Col00"
                className="sf_colsIn container"
                data-sf-element="Container"
                data-placeholder-label="Container"
              >
                <div className="check-ul-styling">
                  <div>
                    <h2>Build your roadmap to entrepreneurial success</h2>
                    <p>
                      Small and medium enterprises are the growth engines of our
                      economy and this is why we have made it our mission to
                      support them.
                    </p>
                    <div>
                      <br />
                    </div>
                    <h3>Punch</h3>
                    <p>
                      It does not just take a good idea to build a successful
                      business, as beautiful as the sentiment is. It takes
                      knowing the right people, having the right information,
                      knowing where to find the right tools. When it&#39;s all
                      said and done, entrepreneurs need to be connected to the
                      right network. This is exactly what MCB’s SME platform,
                      Punch, promises to deliver. Visit and register to punch.mu
                      today!
                    </p>
                    <div>
                      <br />
                    </div>
                    <h4>Discover Punch</h4>
                    <div
                      data-sf-ec-immutable=""
                      contentEditable="false"
                      style={{ width: 640, height: 360 }}
                    >
                      <div data-sf-disable-link-event="">
                        <div>
                          <iframe
                            title="Punch.mu"
                            width={640}
                            height={360}
                            src="https://www.youtube.com/embed/InYDut63unE?enablejsapi=1&origin=https%3A%2F%2Fmcbmu-aut.sitefinity.cloud&widgetid=1"
                            sandbox="allow-scripts allow-same-origin"
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row mt-50" data-sf-element="Row">
            <div
              id="Main_C004_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            >
              <div className="container">
                <div className="row">
                  <div className="col">
                    <div className="card card-cta-red border-radius-0 bg-color-light box-shadow-1 box-shadow-1-hover">
                      <div className="card-body">
                        <h3 className="card-title mb-1  font-weight-normal text-7 text-transform-none">
                          Punch
                        </h3>
                        <p className="card-text mt-3 lead font-weight-medium text-4">
                          Punch.mu is a thriving online platform where local
                          entrepreneurs make meaningful connections, find
                          solutions to their business challenges and are
                          inspired to harness new resources for growth. Be part
                          of this community.
                        </p>
                        <a
                          href="https://www.punch.mu/"
                          target="_blank"
                          className="mb-1 mt-1 me-1 btn btn-primary"
                        >
                          Sign up now{" "}
                          <i className="fas fa-angle-right icons ms-1" />{" "}
                        </a>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="row mt-50" data-sf-element="Row">
            <div
              id="Main_C005_Col00"
              className="sf_colsIn col-lg-12"
              data-sf-element="Column 1"
              data-placeholder-label="Column 1"
            ></div>
          </div>
        </div>
        {/* Footer */}
        <Footer />
        {/* Footer */}
      </div>{" "}
      {/* Vendor */} {/* Theme Base, Components and Settings */}{" "}
      {/* Current Page Vendor and Views */} {/* Demo */} {/* Theme Custom */}{" "}
      {/* Theme Initialization Files */}
      {/* Mirrored from mcb.mu/sme/sme-connect by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 05 Jan 2024 06:43:57 GMT */}
    </>
  );
};
export default SmeConnect;
